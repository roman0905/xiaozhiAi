...
class ConnectionHandler:
    ...
    def chat(self, query, depth=0):
        ...
        try:
            ...
            llm_prepare_start = time.monotonic()
            ...
            llm_prepare_elapsed = time.monotonic() - llm_prepare_start
            log_latency("llm_prepare", turn_id, llm_prepare_elapsed, depth=depth, text=query)
        except Exception as e:
        ...
        try:
            for response in llm_responses:
                if not _llm_first_token_logged:
                    log_latency("llm_first_token", turn_id, time.monotonic() - _llm_stream_start, depth=depth, text=query)
            ...
            return
        _llm_stream_elapsed = time.monotonic() - _llm_stream_start
        log_latency("llm_stream_total", turn_id, _llm_stream_elapsed, depth=depth, text=query)
        # 处理function call
        if tool_call_flag:
            bHasError = False
            # 处理基于文本的工具调用格式
            ...
            if not bHasError and len(tool_calls_list) > 0:
                ...
                _tool_exec_elapsed = time.monotonic() - _tool_exec_start
                log_latency(
                    "tool_exec_total",
                    turn_id,
                    _tool_exec_elapsed,
                    count=len(futures_with_data),
                    depth=depth,
                )
