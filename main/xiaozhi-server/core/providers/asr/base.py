...
class ASRProviderBase(ABC):
    ...
    async def handle_voice_stop(self, conn: "ConnectionHandler", asr_audio_task: List[bytes]):
        ...
        try:
            ...
            asr_infer_start = time.monotonic()
            ...
            asr_infer_elapsed = time.monotonic() - asr_infer_start
            log_latency("asr_infer", turn_id, asr_infer_elapsed)
            ...
            if isinstance(raw_text, dict):
                ...
                _asr_text_for_log = content_for_length_check
            else:
                # 其他 ASR 返回的纯文本
                if raw_text:
                ...
                _asr_text_for_log = raw_text if isinstance(raw_text, str) else ""

            # 性能监控
            total_time = time.monotonic() - total_start_time
            logger.bind(tag=TAG).debug(f"总处理耗时: {total_time:.3f}s")
            log_latency("asr_total", turn_id, total_time, text=_asr_text_for_log)
